...
\(r \equiv 3\), all-inert global & \(\geq\!3\) factors; \(\leq\!2\) with \(d \in \{57,67,331\}\); no danger triple identified for other \(d \leq 1000\) & Thm~\ref{thm:multi-factor}$^\dagger$ \\
...
\multicolumn{3}{@{}l}{\small $^\dagger$The two real danger triples at \(d = 57, 67\) are disposed of computationally at secondary factors (\S8.8).}\\
\end{tabular}
\end{center}
...
\item \textbf{Order-parameter bound for inert factors:} for
  ...
  order parameters \(d \leq 43\) (Theorem~\ref{thm:pell-exclusion});
  a universal lower bound on \(d\) growing with \(p\) would close
  this branch entirely. Short of that,
  Theorem~\ref{thm:multi-factor} shows that any composite
  counterexample with all factors inert requires \(\geq\!3\) prime
  factors, with at most two having \(d \in \{57, 67, 331\}\) and at
  least one requiring a danger triple
  (Definition~\ref{def:danger-triple}) at some \(d\) outside this
  set; a primitive-divisor survey of all 79 admissible
  \(d \leq 1000\) (the parity obstruction of \S\ref{sec:nct}
  eliminates the rest) identifies only five danger triples, three
  of them phantom (\(W_p\) prime), and none with
  \(d > 683\). The pure
  Class~III subclass of the surviving single-factor configurations
  has density zero (Proposition~\ref{prop:density-zero},
  unconditional); the full residual gap is discussed in \S9,
  Problem~2.
\end{enumerate}
...
structurally narrow: the multi-factor reduction
(Theorem~\ref{thm:multi-factor}) constrains any all-inert
counterexample to a configuration with no known precedent, and
all 684{,}965{,}381 inert factors of
...
(iii)~Immediate from \(W_p \equiv 3 \equiv 3^1 \pmod{8}\).\end{proof}
...
FactorDB, 1 itself prime), contributing 61 primes (the largest
a 371-digit cofactor of \(\Phi_{2065}(2)\)) verified by APR-CL
\cite{ref6}. BLS witnesses \(a \in \{2, 3\}\) and discriminant
...
684{,}965{,}381-factor survey of \S8.5.
...
\(W_p \equiv 3 \pmod{8}\) (Lemma~\ref{lem:W-mod-8}), \(t\) is odd;
...
illustrate the structural gap at large~\(d\) discussed in~\S9,
Problem~2.
...
The factorisation data are in the supplementary repository.
...
handles all admissible \(d \leq 43\). Beyond \(d = 43\), a primitive-divisor
survey of all 79 admissible
\(d \leq 1000\) identifies five danger triples
(Definition~\ref{def:danger-triple}), at
\(d \in \{57, 67, 331, 683\}\): three are phantom (\(W_p\) prime)
and two are disposed of at secondary factors (\S8.8). No danger
triple is identified at any other admissible \(d \leq 1000\). By
Theorem~\ref{thm:multi-factor}, any composite \(W_p\) globally
passing Condition~(II) with all factors inert requires at least one
danger triple at some admissible \(d\) outside \(\{57, 67, 331, 683\}\).
A rough heuristic, modelling each primitive inert divisor of
\(V_{2d}\) independently, gives the expected total number of
danger triples with \(d > D\) as
\[
...
and \(|V_{2d}| \sim (1+\sqrt{2})^{2d}\).
The series converges: for admissible prime \(d = q\),
\(M_d = \operatorname{ord}_q(2)/2 \sim q/2\), so the \(d\)-th term
is \(O((\ln\ln d)^2/d^2)\); for composite admissible \(d\) the lcm
\(M_d\) grows at least as fast. Numerical evaluation at
\(D = 1000\) yields \(E \approx 0.003\), consistent with the zero
count observed (see the supplementary scripts for details).
...
danger-triple exclusion for large \(d\) is an open question.
...
Lemmermeyer \cite[\S\S5.3, 5.7]{ref13}). The self-contained
...
survey, the primitive-divisor survey (\S8.3), the
