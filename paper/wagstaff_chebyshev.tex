...
\author{Alexey Dolotov\thanks{Email: \texttt{alexey@dolotov.com}}}
\date{}
...
\begin{abstract}
The Chebyshev base \(\omega_3 = 3 + 2\sqrt{2}\) and the identity \(W_p + 1 = 4W_{p-2}\) yield an unconditional \(N+1\) primality criterion for the Wagstaff numbers \(W_p = (2^p + 1)/3\) (Theorem~\ref{thm:criterion}); combined with a companion \(N-1\) criterion (Theorem~\ref{thm:combined}) and the Brillhart--Lehmer--Selfridge framework, this gives self-contained primality proofs of \(W_{2617}\), \(W_{10501}\), and \(W_{12391}\), independent of the ECPP proofs in the literature. For the sufficiency of the single Chebyshev congruence \(\omega_3^{(W_p+1)/2} \equiv -1 \pmod{W_p}\) on a hypothetical composite \(W_p\) (Conjecture~\ref{conj:sufficiency}), we show that Condition (I) holds automatically (Theorem~\ref{thm:cond-I}), exclude \(r \equiv 5, 7 \pmod{8}\) (Lemma~\ref{lem:mod-8-exclusion}), and reduce the split branch \(r \equiv 1 \pmod{8}\) to the nonexistence of compatible Pell triples (NCT), proved for all rank parameters \(d \leq 81\) (Theorem~\ref{thm:NCT-81}) --- covering every one of the 185 known split factors. For the inert branch we prove that \(r \equiv 3 \pmod{8}\) cannot pass Condition (II) whenever the order parameter satisfies \(d \leq 43\) (Theorem~\ref{thm:pell-exclusion}).
...
The paper delivers two distinct results, summarised in \S\S1.1
and 1.2 below.
...
to two narrow residual problems.} The sufficiency of the single
...
composite \(W_p\) consistent with the conjecture (\S\S8.1, 8.5).
...
give self-contained primality proofs of
...
\subsection{Structural reduction of Conjecture~\ref{conj:sufficiency}}
...
\item \(r \equiv 1 \pmod{8}\): \textbf{reduced to Conjecture~\ref{conj:R3}} via Theorem~\ref{thm:R3-conditional}.
  ...
  parameters \(d \leq 81\) (Theorem~\ref{thm:NCT-81}). This covers every
  one of the 185 known split factors.
...
\begin{center}
\begin{tabular}{p{6.2cm}p{5cm}l}
\hline
Component & Status & Reference \\
\hline
BLS proof of \(W_{2617}\), \(W_{10501}\), \(W_{12391}\) & Unconditional & Thm~\ref{thm:W2617}--\ref{thm:W12391} \\
Condition (I) holds at every prime divisor of \(W_p\) & Unconditional & Thm~\ref{thm:cond-I} \\
\(r \equiv 5, 7 \pmod{8}\) cannot divide \(W_p\) & Unconditional & Lem~\ref{lem:mod-8-exclusion} \\
\(r \equiv 1\): Conjecture~\ref{conj:R3} implies failure of Condition (II) & Unconditional & Thm~\ref{thm:R3-conditional} \\
\(r \equiv 1\), \(d \leq 81\) (covers all 185 known split factors) & Unconditional & Thm~\ref{thm:NCT-81} \\
\(r \equiv 1\), \(83 \leq d \leq 200\) & Computational\footnotemark & \S8.4 \\
\(r \equiv 3\), \(G_r \leq 43\) cannot pass Condition (II) & Unconditional & Cor~\ref{cor:small-Gr} \\
\(r \equiv 3\), \(G_r > 43\) (general inert) & Density zero for natural subclass (unconditional); open pointwise & Proposition~\ref{prop:density-zero} \\
Conjecture~\ref{conj:sufficiency} (full sufficiency of Condition (II)) & Open & \S9 \\
\hline
\end{tabular}
\end{center}
\footnotetext{``Computational'' here means an exhaustive finite search, fully deterministic and reproducible from the supplementary scripts; it is not a heuristic or probabilistic verification.}

All scripts, factorization files, BLS certificates, and verification
logs referenced in this paper are available in the supplementary
repository (see \S8.7 and ``Supplementary material'' at the end of the
References).
...
\item \textbf{Conjecture~\ref{conj:R3} at split factors:} at every split prime divisor
  ...
  \S5.2), and in general it is equivalent (Proposition~\ref{prop:rank-equivalence}) to the
  nonexistence of compatible Pell triples (NCT), which we prove for
  all \(d \leq 81\) (Theorem~\ref{thm:NCT-81}) --- covering every one of the 185
  known split factors --- and verify computationally for the 11
  parity-unblocked values \(d \in [83, 200]\) (\S8.4). The asymptotic
  gap (large \(d\)) is open.
\item \textbf{Order-parameter bound for inert factors:} for
  \(r \equiv 3 \pmod{8}\), Pell-sequence exclusion rules out all
  order parameters \(d \leq 43\) (Theorem~\ref{thm:pell-exclusion}); a universal lower
  bound on \(d\) growing with \(p\) would close this branch.
  Proposition~\ref{prop:density-zero} shows unconditionally that a natural subclass
  of the surviving configurations has natural density zero;
  a pointwise closure of the full residual set appears to require
  structural input beyond the Chebotarev density theorem
  (\S9, Problem 2).
\end{enumerate}
...
structurally narrow: all 684{,}965{,}381 inert factors of
composite \(W_p\) tested in \S8.5, together with all 36 known
Wagstaff primes and probable primes (\S8.1), are consistent with
the conjecture.
...
we write \(G_r := \gcd((r+1)/4,\, W_{p-2})\); see \S6.1 for the
role this quantity plays in the gap structure.
...
\(r \mid 3\), contradicting \(\gcd(W_p, 3) = 1\). So
\(\operatorname{ord}_r(2) = 2p\), and \(2p \mid r-1\) by Fermat.\end{proof}


\begin{lemma}\label{lem:W-mod-8}
For all odd primes \(p \geq 3\): \(W_p \equiv 3 \pmod{8}\).
\end{lemma}


\begin{proof}
Since \(p\) is odd: \(2^p \equiv 0 \pmod{8}\), so
\(2^p + 1 \equiv 1 \pmod{8}\) and
\(3W_p = 2^p + 1 \equiv 9 \pmod{24}\), giving
\(W_p \equiv 3 \pmod{8}\).\end{proof}


\begin{lemma}\label{lem:mod-8-exclusion}
No prime \(r \equiv 5 \pmod{8}\) or \(r \equiv 7 \pmod{8}\)
can divide \(W_p\).
\end{lemma}


\begin{proof}
By Lemma~\ref{lem:ord-2}, \(\operatorname{ord}_r(2) = 2p\) with
\(v_2(2p) = 1\).

\emph{Case \(r \equiv 7\):} By quadratic reciprocity, \((2/r) = 1\), so
\(\operatorname{ord}_r(2) \mid (r-1)/2\). Since \(v_2(r-1) = 1\),
\((r-1)/2\) is odd, so \(\operatorname{ord}_r(2)\) is odd. But
\(2p\) is even. Contradiction.

\emph{Case \(r \equiv 5\):} Here \((2/r) = -1\), so by Euler's criterion
\(2^{(r-1)/2} \equiv -1 \pmod{r}\), giving
\(v_2(\operatorname{ord}_r(2)) = v_2(r-1) = 2\).
But \(v_2(2p) = 1 \neq 2\). Contradiction.
\end{proof}

\begin{remark}
Since \(W_p \equiv 3 \pmod{8}\) and every prime factor
is \(\equiv 1\) or \(3 \pmod{8}\), the count of factors
\(\equiv 3\) is necessarily odd.
\end{remark}
...
\begin{remark}
Lemma~\ref{lem:mod-8-exclusion} excludes \(r \equiv 5, 7 \pmod{8}\) from
dividing any \(W_p\), irrespective of Condition~(II). Lemma~\ref{lem:v2-classification}
is the complementary local statement controlling the 2-adic
valuation of \(\operatorname{ord}_r(\omega_3)\) at the surviving
factor classes --- exact at inert factors (\(r \equiv 3\)) and
bounded in the split case (\(r \equiv 1\)). It is invoked in \S6.1
to establish the form \(\operatorname{ord}(\omega_3) = 4g\) (with
\(g\) odd) at inert factors.
\end{remark}
...
The base \(a = 3\) is the unique integer
\(a \geq 2\) with \(a^2 - 1\) a power of 2. Indeed,
\(a^2 - 1 = 2^k\) with \(k \geq 1\) is equivalent to
\((a-1)(a+1) = 2^k\); this forces \(a - 1\) and \(a + 1\) to both
be powers of 2 differing by 2, so \(\{a-1, a+1\} = \{2, 4\}\) and
\(a = 3\). (More generally, the equation \(x^2 - 2^k = 1\) in
integers \(x \geq 2\), \(k \geq 1\) has only the solution
\(x = 3\), \(k = 3\), a special case of Mihailescu's theorem on
Catalan's conjecture \cite{ref14}.) The base \(a = 3\)
therefore gives the only universal \(N+1\) condition of this form.
\end{remark}
...
\(N \equiv 3 \pmod{4}\), \(N\) is not a perfect square, so
...
\(s \equiv 1 \pmod{r+1}\).
Since \(s \geq r\), the first candidate is \(s = r + 2\)
(larger values give \(N > (r+1)^2 > N\)). So
\[
N = r(r+2), \qquad N + 1 = (r+1)^2.
\]
Since \(N + 1 = 4W_{p-2}\): \(W_{p-2} = ((r+1)/2)^2\). But
\(W_{p-2} \equiv 3 \pmod{8}\) (Lemma~\ref{lem:W-mod-8}), and a perfect square is
...
(\S8.1). For larger \(p\), the factored portion satisfies
...
\subsection{The case \(W_{p-2}\) prime}
...
\subsection{BLS proof of \(W_{2617}\)}
...
certificates \texttt{bls\_certificate\_w*.json} are documented in \S8.7.
...
\begin{proof}
Set \(N = W_{2617}\), a 788-digit number.
We apply the BLS \(N-1\) criterion \cite{ref3}
(Remark~\ref{rem:bls-general}). The exponent
\(p - 1 = 2616 = 2^3 \cdot 3 \cdot 109\) has 16 divisors;
the cyclotomic decomposition \(2^{2616} - 1 = \prod_{d \mid 2616} \Phi_d(2)\)
yields 12 factorable terms (\(d \leq 654\), at most 66 digits each),
contributing 22 distinct prime factors. The factored portion \(F\)
of \(N-1\) satisfies \(F > N^{1/3}\) with a margin of 15 bits. For
each prime \(q \mid F\), the witness \(a = 2\) (or \(a = 3\))
satisfies \(a^{N-1} \equiv 1\) and
\(\gcd(a^{(N-1)/q} - 1, N) = 1\). The BLS discriminant is not a
perfect square, excluding the two-factor case.
Condition (II) is also verified:
\(\omega_3^{(N+1)/2} \equiv -1 \pmod{N}\).
\end{proof}

\subsection{BLS proof of \(W_{10501}\)}
...
\begin{proof}
Set \(N = W_{10501}\), a 3161-digit number. We apply the BLS
\(N-1\) criterion \cite{ref3} (Remark~\ref{rem:bls-general}).
The exponent \(p - 1 = 10500 = 2^2 \cdot 3 \cdot 5^3 \cdot 7\)
has 48 divisors. Of the 48 cyclotomic factors \(\Phi_d(2)\),
43 are factored (38 by direct computation, 5 from the Cunningham
tables \cite{ref4}), contributing 103 distinct prime factors
whose product \(F\) exceeds \(N^{1/3}\) by 1086 bits. Each
factor's primality is verified by APR-CL \cite{ref6}. BLS witnesses
\(a \in \{2, 3, 5\}\) satisfy the required conditions for all
\(q \mid F\), and the BLS discriminant is not a perfect square,
excluding the two-factor case. The complete cyclotomic
decomposition and factor list are in the supplementary certificate
\texttt{bls\_certificate\_w10501.json}.
\end{proof}

\subsection{BLS proof of \(W_{12391}\)}
...
\begin{proof}
Set \(N = W_{12391}\), a 3730-digit number. We apply the BLS
\(N-1\) criterion \cite{ref3} (Remark~\ref{rem:bls-general}).
The exponent \(p - 1 = 12390 = 2 \cdot 3 \cdot 5 \cdot 7 \cdot 59\)
has 32 divisors. Of the 32 cyclotomic factors \(\Phi_d(2)\),
28 are factored (19 by direct computation, 1 itself prime, and 7
from the Cunningham tables \cite{ref4} and FactorDB),
contributing 61 distinct prime factors (the largest a 371-digit
cofactor of \(\Phi_{2065}(2)\)) whose product \(F\) exceeds
\(N^{1/3}\) by 953 bits. Each factor's primality is verified by
APR-CL \cite{ref6}. BLS witnesses \(a \in \{2, 3\}\) satisfy
the required conditions for all \(q \mid F\), and the BLS discriminant
is not a perfect square, excluding the two-factor case. The complete
factor list is in \texttt{bls\_certificate\_w12391.json}.
\end{proof}
...
Among all 36 known Wagstaff primes and probable primes with
\(p > 2617\), the exponents \(p = 10501\) and \(p = 12391\) are
...
since \(p\) is odd) and is odd (numerator and denominator are both odd).
...
\begin{remark}
Squarefreeness of \(W_p\) is not known for general \(p\).
Theorem~\ref{thm:cond-I} is the unconditional structural statement; Corollary~\ref{cor:cond-I-crt}
is a practical convenience for testing. All subsequent sections use
only the per-prime version (Theorem~\ref{thm:cond-I}), so the results of \S\S5--7
are unconditional regardless of whether \(W_p\) is squarefree.
\end{remark}
...
factor \(r \mid W_p\) modulo 8. By Lemma~\ref{lem:mod-8-exclusion}, a composite \(W_p\)
passing Condition (II) must have all prime factors \(\equiv 1\) or
\(3 \pmod{8}\), so the composite analysis reduces to two
branches:
...
\textbf{Inert} & \(r \equiv 3 \pmod{8}\) & \(d \leq 43\) proved; residual subclass density zero (\S6.6) \\
\hline
\end{tabular}
\end{center}
...
\section{The \(r \equiv 1 \pmod{8}\) branch}
...
and \(d \mid W_{p-2}\). After the descent of \S5.1 and the explicit
Cases A, B, and \(4U_p^2 \equiv 1\) of \S5.2, \S\S5.3--5.5 identify
...
and prove NCT for \(d \leq 81\) (Theorem~\ref{thm:NCT-81}) --- covering every
one of the 185 known split Wagstaff factors.
...
\begin{proof}
By Fermat: \(W_p \equiv 1 \pmod{p}\),
so \(W_{p-2} = (W_p + 1)/4 \equiv (p+1)/2 \not\equiv 0 \pmod{p}\).\end{proof}
...
follows from the Pell recurrences \(V_p = V_{p-1} + 4U_{p-1}\) and
\(4U_p = 4U_{p-1} + 2V_{p-1}\).\end{proof}
...
\tag{$\ast$}
\]
Verification: \((1-\zeta_8)^2 = (1+i)(1-\sqrt{2})\),
so \((1-\zeta_8)^2 \cdot \zeta_8^3 = 2 - \sqrt{2}\), and
\((2 - \sqrt{2})(1+\sqrt{2}) = \sqrt{2}\). \checkmark
...
\subsection{No Compatible Triples}
...
\(p - 2\) is odd). This parity obstruction eliminates 80.6\% of odd
...
\(r \equiv 1 \pmod{8}\) is checked: in all cases,
\(\operatorname{ord}_r(2)\) is either odd or
\(\operatorname{ord}_r(2)/2\) is composite, so no compatible triple
exists. Complete factorizations are in the supplementary
repository.
\end{proof}
Combined with Theorems~\ref{thm:case-A}--\ref{thm:case-4U}, this proves Conjecture~\ref{conj:R3}
unconditionally for every one of the 185 known split Wagstaff
factors: Cases A, B, and the \(4U_p^2 \equiv 1\) subcase account
for 35\% of instances (Theorems~\ref{thm:case-A}, \ref{thm:case-B}, \ref{thm:case-4U}), and all remaining
general Case C instances have rank parameter \(d \leq 81\) and
are therefore handled by Theorem~\ref{thm:NCT-81}. See \S8.2 for the full
distribution by proof method.
...
\section{The \(r \equiv 3 \pmod{8}\) branch}
...
\begin{proof}
By Theorem~\ref{thm:pell-divisibility}, such \(r\) would divide \(N_d\).

For \(d = 1\): \(V_4 = 34\), \(U_4 = 12\).
\(N_1 = \gcd(18, 12) = 6 = 2 \cdot 3\). The only prime
\(\equiv 3 \pmod{8}\) is 3. Since \(\gcd(W_p, 3) = 1\) for
\(p \geq 5\): no valid \(r\) exists.

For \(d = 3\): \(V_{12} = 39202\), \(U_{12} = 13860\).
\(N_3 = \gcd(19602, 13860) = 198 = 2 \cdot 3^2 \cdot 11\). Primes
...
For \(d \in \{9, 11, 19, 27, 33, 43\}\): \(N_d\) is computed and
completely factored (the largest, \(N_{43}\), has 55 digits). Each
...
analysed in \S8.5. A natural subclass of the residual configurations
...
one --- is discussed in \S9, Problem 2.

\textbf{Computational status.} Across the 684{,}965{,}381 inert factors
of composite \(W_p\) tested in \S8.5 (\(r < 10^{12}\),
\(p < 5 \times 10^{11}\)), no configuration globally passes
Condition (II); in every case the failure is witnessed either by
the unconditional Theorems~\ref{thm:parity-obstruction} and~\ref{thm:pell-exclusion} at the factor itself or by a
cross-factor argument at a secondary prime divisor of the same
\(W_p\) (\S8.8).
...
the \emph{configuration}, succeeding for all \(g \leq 43\) because the
numbers \(N_d\) are small enough to factor completely. Beyond
...
Let \(W_p\) be composite and suppose Condition (II) holds.
For each prime factor \(r \mid W_p\):

\begin{center}
\begin{tabular}{lll}
\hline
Factor class & Status & Reference \\
\hline
\(r \equiv 5, 7 \pmod{8}\) & Impossible & Lemma~\ref{lem:mod-8-exclusion} \\
\(r \equiv 1\), Case A (\(r \mid U_p\)) & Blocked & Theorem~\ref{thm:case-A} \\
\(r \equiv 1\), Case B (\(r \mid V_p\)) & Blocked & Theorem~\ref{thm:case-B} \\
\(r \equiv 1\), \(4U_p^2 \equiv 1\) & Blocked & Theorem~\ref{thm:case-4U} \\
\(r \equiv 1\), general Case C & Proved for \(d \leq 81\); verified for \(83 \leq d \leq 200\); open & Theorem~\ref{thm:NCT-81}, \S8.4 \\
\(r \equiv 3\), \(G_r \leq 43\) & Blocked & Corollary~\ref{cor:small-Gr} \\
\(r \equiv 3\), \(G_r > 43\) & Density zero for natural subclass (unconditional); pointwise open & Proposition~\ref{prop:density-zero} \\
\hline
\end{tabular}
\end{center}


The two open problems are NCT for general \(d\) (\S5.5) and the
universal order-parameter bound for inert factors (\S6.4). Both
require unconditional lower bounds on multiplicative orders, but
are structurally independent: NCT is a finite local verification
at each \(d\) (proved through \(d = 81\)), while the
order-parameter bound is asymptotic and currently closed only in
density (Proposition~\ref{prop:density-zero}).
...
\hline
Case A (\(r \mid U_p\)) & 0\% & Theorem~\ref{thm:case-A} \\
Case B (\(r \mid V_p\)) & 6\% & Theorem~\ref{thm:case-B} \\
Case C, \(4U_p^2 \equiv 1\) & 29\% & Theorem~\ref{thm:case-4U} \\
Case C, general (\(d \leq 81\)) & 65\% & Theorem~\ref{thm:NCT-81} \\
\hline
\end{tabular}
\end{center}


No Case A instance occurs among the 185 surveyed factors. Case A
(\(r \mid U_p\)) is algebraically the most natural way for Conjecture~\ref{conj:R3} to
hold --- \(p \mid \rho(r)\) implies \(p \mid \operatorname{ord}_r(\alpha)\)
directly --- but it does not appear empirically in the Wagstaff
setting and may in fact be impossible there; we include it as a
complete-coverage proof case rather than as a witnessed
configuration.
...
\(d \in \{83, 99, 107, 121, 129, 131, 139, 163, 171, 177, 179\}\)
...
all prime factors of observed \(G_r\) lie in \(\{3, 11, 19, 43\}\),
consistent with the structure of \(A\).

\textbf{Extended inert-factor survey.} A segmented sieve
enumerated every prime \(r \equiv 3 \pmod{8}\) with
...
of \S9 Problem 2 --- are recorded in \S8.8.
...
output \textbf{probable prime} on a composite \(W_p\) (\S8.1, \S8.5).
...
\hline
Theorem~\ref{thm:criterion} & Factorization of \(W_{p-2}\) & Each application (\S8.1) \\
...
Theorem~\ref{thm:pell-exclusion} & Complete factorization of \(N_d\) for each admissible \(d \leq 43\); for each prime \(r \equiv 3\,(8)\) dividing \(N_d\), verify the same three conditions & 6 values of \(d\); largest factorization: \(N_{43}\) (55 digits) \\
\hline
\end{tabular}
\end{center}
...
\item \emph{Pre-known large factorizations}: the Cunningham project tables
  ...
  factors \(\Phi_d(2)\) with \(d \leq 1200\), and FactorDB for
  selected larger \(d\) (specifically the factorizations of
  \(\Phi_{2065}(2)\) and \(\Phi_{2478}(2)\) used in
  Theorem~\ref{thm:W12391}); each retrieved factor is independently re-verified
  for primality before use.
...
The driver scripts \texttt{bls\_n\_minus\_1\_w2617.py},
\texttt{bls\_n\_minus\_1\_w10501.py}, and \texttt{bls\_n\_minus\_1\_w12391.py} in the
...
The extended inert-factor survey of \S8.5 was carried out on a
...
\item \emph{Small-\(d\) gap at \(d = 57\).}
  ...
  danger configuration at \(d = 57\) identified in \S9,
...
Theorem~\ref{thm:NCT-81} and Theorem~\ref{thm:A.8}; we report it only for
completeness.}) are recorded in the supplementary repository.

\emph{Secondary-factor closures (\S9, Problem 2).} The dangerous
...
secondary factors; no unconditional argument is known for either
case. Their role is narrow: they are the only two configurations
the single-factor argument of \S\S5--6 currently fails to rule out,
and the secondary-factor argument disposes of both.
...
(Theorem~\ref{thm:NCT-81}); verified computationally through \(d = 200\) (\S8.4).
The parity obstruction eliminates 80.6\% of \(d\); the remaining
...
\textbf{Problem 2} (Universal order-parameter bound for inert factors). For
\(r \equiv 3 \pmod{8}\) dividing a composite \(W_p\), prove that
\(\operatorname{ord}_r(\omega_3)/4 \nmid W_{p-2}\). Theorem~\ref{thm:pell-exclusion}
handles all admissible \(d \leq 43\). The bound is sharp: \(N_{57}\)
and \(N_{67}\) each have a surviving single-factor configuration
(\S8.8). Both are disposed of computationally at secondary factors;
no unconditional argument is known for either. A natural subclass of
the residual configurations has density zero (Proposition~\ref{prop:density-zero},
unconditional), but a naive Hooley-type argument does not extend
this to pointwise closure (\S6.6).
...
(Lemma~\ref{lem:v2-classification}), the Pell exclusion numbers \(N_d\) (Definition after
Theorem~\ref{thm:pell-divisibility}), and the compatible-triple formulation of Conjecture~\ref{conj:R3}
...
(i.e., \(b \neq 3\) for the Remark~\ref{rem:uniqueness} base), so the Pell
sequences are replaced by the Lucas sequences of
\(\mathbb{Z}[\sqrt{D}]\).
\end{remark}
...
identity in the Pell representation \(r = a^2 - 2b^2\) (\SA.1,
...
Proposition~\ref{prop:A.4} is classical, due originally
to Dirichlet as part of his simplification of Gauss's biquadratic
reciprocity. In the standard list of equivalent formulations at a
prime \(p = a'^2 + b'^2 = c^2 + 2d^2 = a^2 - 2b^2 \equiv 1 \pmod 8\)
(\(a'\) odd), the three parallel identities
\((2/p)_4 = (-1)^{b'/4} = (2/c) = (-2/a)\)
all appear in Dirichlet's treatment (see, e.g., Ireland and
Rosen \cite[Ch.~11]{ref11} and Lemmermeyer's historical account
\cite[\S\S5.3, 5.7]{ref13}; the last of the three is the statement of
Proposition~\ref{prop:A.4}). The self-contained derivation via
Lemmas~\ref{lem:A.1}--\ref{lem:A.3} is included only for the reader's convenience.
A symmetric companion identity in the \(\mathbb Z[\sqrt{-2}]\)
representation \(r = c^2 + 2d^2\) (\(c\) odd) reads
\(2^{(r-1)/4} \equiv (2/c) \pmod r\), and is proved by the same
argument with sign changes.
\end{remark}
...
\bibitem{ref5} K. S. Chua, \emph{Chebyshev polynomials and higher order Lucas--Lehmer algorithm}, preprint, 2020, arXiv:2010.02677.
...
A repository accompanies this paper, containing:
\begin{enumerate}
\item BLS primality certificates and self-contained
verification scripts for \(W_{2617}\), \(W_{10501}\), and
\(W_{12391}\), including timestamped computation logs and
APR-CL primality proofs for all factors exceeding 82 digits
(Theorems~\ref{thm:W2617}--\ref{thm:W12391});
\item factorization files for \(U_{4d}\) (\(d \leq 81\)) and
\(N_d\) (\(d \leq 43\)) with verification logs
(Theorems~\ref{thm:NCT-81},~\ref{thm:pell-exclusion});
\item the primitive-divisor survey outputs for \(q \leq 1000\) (\S8.3);
...
\S8.5), recording \(p\), \(r\), \(\operatorname{ord}_r(2)\),
\(G_r\), and the Condition~(II) outcome; and
\item code for the Chebyshev primality test (\S8.6).
\end{enumerate}
...
The archive of record is on Zenodo (\href{https://doi.org/10.5281/zenodo.19496206}{10.5281/zenodo.19496206}); a development mirror is on GitHub at
