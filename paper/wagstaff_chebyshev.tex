...
\begin{abstract}
The Chebyshev base \(\omega_3 = 3 + 2\sqrt{2}\) and the identity \(W_p + 1 = 4W_{p-2}\) yield an unconditional \(N+1\) primality criterion for the Wagstaff numbers \(W_p = (2^p + 1)/3\); combined with a companion \(N-1\) criterion and the Brillhart--Lehmer--Selfridge framework, this gives self-contained primality proofs of \(W_{2617}\), \(W_{10501}\), and \(W_{12391}\), independent of the ECPP proofs in the literature. For the sufficiency of the single Chebyshev congruence \(\omega_3^{(W_p+1)/2} \equiv -1 \pmod{W_p}\) on a hypothetical composite \(W_p\), we show that Condition~(I) holds automatically, exclude prime factors \(r \equiv 5, 7 \pmod{8}\), and reduce the split branch \(r \equiv 1 \pmod{8}\) to the nonexistence of compatible Pell triples (NCT), proved for all rank parameters \(d \leq 81\); Conjecture~R3 is verified computationally at all 185 known split factors. For the inert branch we prove that \(r \equiv 3 \pmod{8}\) cannot pass Condition~(II) whenever the order parameter satisfies \(d \leq 43\).
...
below. Sections~2--3 are self-contained and establish~(A);
Sections~4--7 develop~(B); Section~8 presents the computational
verification.
...
\begin{center}
\small
\begin{tabular}{@{}p{5.5cm}p{4.8cm}l@{}}
\hline
...
\(r \equiv 1\), NCT for \(83 \leq d \leq 200\) & Computational$^*$ & \S8.4 \\
\(r \equiv 1\), Conj.~\ref{conj:R3} (all 185 known) & Computational$^*$ & \S8.2 \\
...
Full Conj.~\ref{conj:sufficiency} & Open & \S9 \\
\hline
...
\item \textbf{Conjecture~\ref{conj:R3} at split factors:} at every split prime divisor
  ...
  for the 11 parity-unblocked values \(d \in [83, 200]\) (\S8.4).
...
\item \textbf{Order-parameter bound for inert factors:} for
  ...
  Proposition~\ref{prop:density-zero} shows unconditionally that the pure Class~III
  subclass of the surviving configurations has natural density zero;
  the full residual set is slightly larger, and its pointwise closure
  appears to require structural input beyond the Chebotarev density
  theorem (\S9, Problem 2).
\end{enumerate}
...
bounded in the split case (\(r \equiv 1\)).
The asymmetry is intrinsic: in the inert case, Frobenius forces
\(\alpha^{r+1} = -1\), which determines \(v_2(\operatorname{ord}(\alpha))\)
exactly; in the split case, \(\alpha\) lies in \(\mathbb{F}_r^{\times}\)
and its order may occupy any divisor of \(r-1\).
The lemma is invoked in \S6.1
...
The complete factor list is in \texttt{bls\_certificate\_w2617.json}.
\end{proof}
...
27 are factored (19 by direct computation, 1 itself prime, and 7
...
\textbf{Inert} & \(r \equiv 3 \pmod{8}\) & \(d \leq 43\) proved; pure Class~III subclass density zero (\S6.6) \\
\hline
\end{tabular}
\end{center}
...
Conjecture~\ref{conj:R3} is verified at all 185 known split factors (\S8.2).
...
\noindent Complete factorizations of each \(U_{4d}\) are in the supplementary
repository.
\end{proof}
...
computational R3 verification (\S8.2).
...
For comparison, the split branch (\S8.8, footnote) admits a naive
...
\begin{center}
\small
\begin{tabular}{@{}p{3.8cm}p{6.2cm}l@{}}
\hline
...
\(r \equiv 3\), \(G_r > 43\) & Natural subclass density zero (unconditional); pointwise open & Proposition~\ref{prop:density-zero} \\
\hline
\end{tabular}
\end{center}


The two open problems are NCT for general \(d\) (\S5.6) and the
...
\item \emph{Small-\(d\) gap at \(d = 57\).}
  ...
  residual configuration at \(d = 57\) identified in \S9,
...
case, and they are not counted among the unconditional results
in the status table of \S1.2. Their role is narrow: they are the only two configurations
...
yields a surviving single-factor configuration observed directly in
the \(r \leq 10^{12}\) survey (\S8.8, witness~3), while \(N_{67}\)
yields a configuration identified through the broader secondary-factor
analysis (\S8.8). Both are disposed of computationally at secondary
factors; no unconditional argument is known for either. The pure
Class~III subclass of the residual configurations has density zero
(Proposition~\ref{prop:density-zero}, unconditional), but the full residual set is
slightly larger, and a naive Hooley-type argument does not extend
the density-zero result to pointwise closure (\S6.6).
The closed form \(N_d = V_d^2 + 2\)
(Proposition~\ref{prop:Nd-closed-form}) exhibits \(N_d\) as a norm
in \(\mathbb{Z}[\sqrt{-2}]\); whether the Zsygmondy theory of
primitive divisors of such norms can yield an asymptotic
compatible-triple exclusion for large \(d\) is an open question.
