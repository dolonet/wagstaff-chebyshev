...
Sharper forms of the chain are also established (\S\ref{sec:main-mpc}, \S\ref{sec:triple-form}): the split-branch residual weakens to a \emph{Mixed Pair Conjecture} implied by NCT for general $d$; the term $W$ decomposes into an explicit super-Wieferich bucket --- closed unconditionally via the Nagell--Ljunggren equation at $x = -2$ --- and a single-exponent bucket; and a triple-count erosion (PPPC3) reduces the role of PPPC from a pair count to a triple count.
...
\hline
NCT, general $d$ \newline (split, $r \equiv 1$) & no compatible triple for any odd $d \leq 200$, nor at any parity-unblocked value of $(200, 400]$ (Thm~\ref{thm:nct99} + twenty-eight complete fixed-$d$ closures, \S\ref{sec:nct-verify}) & all $185$ known split factors satisfy R3 & general odd $d > 400$ \\
...
\hline
\end{tabular}
\caption{State of the three residuals of Theorem~\ref{thm:main-intro}. Conjecture~\ref{conj:suff} follows from all three. NCT-general and PPPC are the two principal gaps; $W = 0$ is the mild third, discharged unconditionally throughout the computational range. The sharpened residuals MPC$^{>}$, X1 of Theorem~\ref{thm:sharpened-chain} are implied by these three as a conjunction (\S\ref{sec:main-reduction}); the former fourth residual $W_1 = 0$ is closed unconditionally (Lemma~\ref{lem:w1-empty}).}
\label{tab:status}
\end{table}
...
(b) Every $p$ counted by $W_1$ has $W_p = r^t$ with $t$ odd, $t \geq 3$, and $r$ a base-$2$ \emph{super}-Wieferich prime $\equiv 3 \pmod 8$; in particular $r > 2^{64}$, and $(2^p + 1)/3 = y^q$ is solvable with integers $y > 1$, $q \geq 3$ --- a Nagell--Ljunggren instance at $x = -2$, excluded unconditionally by Lemma~\ref{lem:w1-empty} below.
...
For every odd prime $p \geq 5$ there are no integers $y > 1$, $q \geq 2$ with $W_p = y^q$. In particular $W_1 = 0$, unconditionally.
\end{lemma}
...
\begin{corollary}\label{cor:w1-zero}
The trichotomy of Theorem~\ref{thm:w-trichotomy}(a) reads $E_3 \leq \Pi + W_2$: the prime-power bucket is empty for every $p$.
\end{corollary}

Theorem~\ref{thm:w-trichotomy}(b) retains independent interest as a cross-check: any hypothetical solution of $W_p = y^q$ would also force a base-$2$ super-Wieferich prime $\equiv 3 \pmod 8$ above $2^{64}$, so the bucket was doubly walled.

\begin{remark}[Bucket-level weakening]\label{rem:bucket-weakening}
With $W_1 = 0$ settled unconditionally (Lemma~\ref{lem:w1-empty}), the bucket-level weakening concerns only $W_2$. Every $p$ counted by $W_2$ has $W_p$ non-squarefree, all-inert and locally passing everywhere, hence is counted by $W$; so
\[
  W = 0 \;\Longrightarrow\; W_2 = 0,
\]
and the residual $W_2 = 0$ is weaker \emph{as a hypothesis} than the blanket $W = 0$ of Theorem~\ref{thm:main}. ($W_1$ needs no hypothesis at all, by Lemma~\ref{lem:w1-empty}.) The weakening is claimed at the bucket level only.
\end{remark}
...
Conjecture~\ref{conj:pppc} are the primes $r = 2 p k + 1 > 10^{12}$ with
$k \equiv p \pmod 4$ (the class forced by $r \equiv 3 \pmod 8$, as in
Theorem~\ref{thm:pair-separation}(1)); the model takes them to arrive
independently with intensity
\[
  \Prob\big[\text{(i)--(iv) at } k\big] \;\approx\;
  \frac{C_{\mathrm{cal}}}{2 k \,\ln(2 p k)},
\]
the factor $1/(2k) = p/(r-1)$ being the order-alignment probability
($\ord_r(2) = 2p$) and $C_{\mathrm{cal}}$ a single calibration constant
absorbing the AP-primality and mod-$8$ corrections. Calibrating on the
measured window mean of \S\ref{sec:empirical},
...
(ratio $0.95$). Integrated over the full factor spectrum this intensity
...
\]
\[
  \delta_{\mathrm{best}}(r) = \delta_{\mathrm{cen}} \cdot
  \min\!\big(1,\, 10^{12}/r\big).
\]
The ceiling is the $95\%$ rule-of-three bound from the above-cutoff
zero-pass record ($369{,}876 + 6{,}559$ tested candidates, $0$ passes,
\S\ref{sec:empirical}); the central value is the only nonzero measured
density --- the Platinum enumeration's $3$ passes among the
$15{,}587{,}021$ rows surviving the $G_r \leq 43$ exclusion
(\S\ref{sec:inert-survey}), at $r \leq 10^{12}$; the best estimate
decays the measured density like $1/r$ beyond the measurement scale
(the alignment mass of divisors $d_r \mid (r+1)/4$ against the fixed
integer $W_{p-2}$), the same structural-decay device as the MPC
...
this paper --- Theorem~\ref{thm:pair-separation}, and the diagonal
closures of Proposition~\ref{prop:diagonal-closures} modulo their stated
certificates, neither credited by the model --- both push the true pair
...
$\nu_{p^\ast} = 9.3 \times 10^{-5}$ (ceiling) $/\; 2.2 \times 10^{-6}$
(central) $/\; 7.8 \times 10^{-9}$ (best), giving per-exponent pair
probabilities
\[
  \frac{\nu_{p^\ast}^2}{2} \;=\;
  4.3 \times 10^{-9} \;\big/\; 2.5 \times 10^{-12} \;\big/\;
  3.1 \times 10^{-17}
  \qquad \text{(ceiling / central / best)}.
\]
...
\emph{Summed expectation.} Under the flat readings the sum over $p$ of
$\nu_p$ --- let alone of $\nu_p^2/2$ --- diverges: with a constant
$\delta$ and $\mu_p \sim \tfrac12 \ln\ln W_p$, single passes recur
infinitely often. This is the model face of
Remark~\ref{rem:triple-status}, which expects the single-pass first
moment to be divergent (the enumeration already exhibits three witness
exponents), and it is why the flat readings are summed only through the
$d$-indexed form of the count, where each order parameter $d$
contributes the convergent single-triple summand $E_1(d) = \tfrac12
(\ln\ln V_{2d})^2 / (2 \ln V_{2d}\, \varphi(L_d))$ of the display above
and a partner factor $\min(1,\, \omega_{\mathrm{in}}(\bar p_d)\,
\delta)$, $\omega_{\mathrm{in}}(x) = \tfrac12(\ln x - \ln\ln x)$, at the
pinned scale $\ln \bar p_d = \ln V_{2d} - \ln 4$ (the same partner
device as the MPC heuristic). The best reading needs no such cap: the
$1/r$ decay makes the $p$-indexed sum $\sum_p \nu_p^2/2$ converge
outright, dominated within a decade of the anchor scale $p^\ast$ ---
exactly where the calibration is measured. The results:
\[
  \mathbb{E}[\Pi] \;\approx\;
...
uncertainty --- it is the cost of extrapolating a condition-(v) density
...
heuristic (whose bracket, in the same framework, is $8.7 \times 10^{-4}$
/ $4.4 \times 10^{-4}$ / $9.8 \times 10^{-10}$). The best-estimate
reading reproduces the $O(10^{-6})$ pair figure quoted in earlier
versions of this section. Constants and calibration gates:
\texttt{scripts/cp395\_pppc\_heuristic\_constants.py} (the script first
reproduces the $E(1000)$ figures of the display above, then the window
means, before computing the bracket). These figures are heuristics only;
they prove nothing about PPPC, which remains an open conjecture, and
Conjecture~\ref{conj:suff} is expected to hold for the structural reason
described below, not because of these numbers.
The rigorous proof of $\Pi = 0$ is open.
...
\emph{Case B: all factors inert.} Then $p$ contributes to $E_3$. By Theorem~\ref{thm:w-trichotomy}, $E_3 \leq \Pi + W_1 + W_2$; PPPC gives $\Pi = 0$; $W_1 = 0$ by Lemma~\ref{lem:w1-empty}; and $W_2 = 0$ by Corollary~\ref{cor:w2-bound} under X1. Hence $E_3 = 0$ --- contradiction.

\emph{Conjunction implication.} Assume PPPC, NCT-general and $W = 0$. NCT-general implies MPC (Proposition~\ref{prop:nct-implies-mpc}), hence MPC$^{>}$; $W = 0$ implies $W_2 = 0$ (Remark~\ref{rem:bucket-weakening}). For X1: suppose Condition~(II) held at every prime factor of the composite $W_{10916765939}$. A split factor would, with Proposition~\ref{prop:rank-equiv}, yield a compatible triple against NCT-general; so every factor would be inert and $p = 10{,}916{,}765{,}939$ would lie in $E_3$, while $E_3 \leq \Pi + W = 0$ under PPPC and $W = 0$ --- contradiction. Hence some factor fails, which is X1. The implication is claimed at the level of the conjunctions; no comparison between individual hypotheses across the two lists is asserted.
\end{proof}

\begin{remark}\label{rem:sharpened-status}
Theorem~\ref{thm:sharpened-chain} is the sharpest conditional statement of this paper with PPPC kept intact: PPPC is unchanged, and each of the other inputs is qualitatively lighter than its counterpart in Theorem~\ref{thm:main}. MPC$^{>}$ is a pair-type statement of the same shape as PPPC (\S\ref{sec:mpc}); and X1 concerns one explicit integer and is discharged by exhibiting any single failing factor of $W_{10916765939}$. The statement $W_1 = 0$, a hypothesis in earlier versions of this chain, is now unconditional (Lemma~\ref{lem:w1-empty}), so the chain reads: Conjecture~\ref{conj:suff} follows from PPPC + MPC$^{>}$ + X1. None of the three inputs is proved here; the chain remains a conditional reduction.
\end{remark}

\subsection{The triple form: eroding the pair count}\label{sec:triple-form}

The pair count $\Pi$ enters Theorem~\ref{thm:sharpened-chain} through the $W$-residual trichotomy (Theorem~\ref{thm:w-trichotomy}), whose proof splits $E_3$ at $|T_p^{>}| \geq 2$. Splitting at $|T_p^{>}| \geq 3$ instead pushes the same structural lemmas one step further: the pair count is replaced by a \emph{triple} count, and the overflow is absorbed by the prime-power bucket $W_1$ --- which is empty by Lemma~\ref{lem:w1-empty} --- one further corner bucket, and an explicit witness-exponent term. The role of PPPC in the chain is thereby eroded from a pair-counting to a triple-counting statement, with the same two computational inputs as Theorem~\ref{thm:sharpened-chain} and no new ones. For the triple count, put
\[
...
\emph{Case B: all factors inert.} Then $p$ contributes to $E_3$. Theorem~\ref{thm:triple-trichotomy}(a) gives $E_3 \leq \Pi_3 + W_1 + W_2' + X_{E_3}$; PPPC3 gives $\Pi_3 = 0$; $W_1 = 0$ by Lemma~\ref{lem:w1-empty}; $W_2' = 0$ by hypothesis; and $X_{E_3} = 0$ by Theorem~\ref{thm:triple-trichotomy}(b) under X1. Hence $E_3 = 0$ --- contradiction.

\emph{Conjunction implication.} PPPC implies PPPC3 ($\Pi = 0$ forces $|T_p^{>}| \leq 1$ for every $p$, hence $\Pi_3 = 0$), and PPPC implies $W_2' = 0$ (Theorem~\ref{thm:triple-trichotomy}(c)); the remaining hypotheses MPC$^{>}$ and X1 are common to the two conjunctions. The final implication then follows from the conjunction implication of Theorem~\ref{thm:sharpened-chain}. As there, the implications are claimed at the level of the conjunctions.
\end{proof}
...
paper --- and assumes none of PPPC, MPC, NCT-general, $W = 0$, or X1.
...
$\Z[\sqrt 2]/(W_p) \to \Z[\sqrt 2]/(r)$ for every prime $r \mid W_p$, as
in the proof of Theorem~\ref{thm:main-mpc}; the residue classes and the
...
$T_p^{>}$ (\S\ref{sec:second-moment}) by (b1) and item (0); there are at
...
distinct count), exactly as in Step 2 of
Theorem~\ref{thm:second-moment}. The pair constraints are
...
in (b5).
\end{itemize}
Items (0), (b1)--(b3), (a1), (a3), (a4) and the pair-separation
congruences of (b5) carry \emph{no} computational input at all: they are
fully algebraic (the perfect-power exclusion rests on the published
Nagell--Ljunggren results \cite{ref:BennettLevin,
ref:BugeaudMignotte2002, ref:BugeaudCaoMignotte2001}).
\end{remark}
