...
| File | Description |
|---|---|
| `nct_certificates.json` | Static fixed-d NCT certificates for all 24 closures in (100,400]: pinned `Psi_{4d}` factorizations, per-factor APR-CL primality, and the certificate disposition. FactorDB-independent; generated by `scripts/cp365_nct_certificate_bundle.py`. |
| `sample_1000.csv` | First 1000 rows of the canonical clean CSV. Smoke test for `scripts/verify_sample.py`. |
| `SHA256SUMS` | SHA-256 hashes of `inert_factors.csv` (full), `sample_1000.csv`, and `nct_certificates.json`. |
| `danger_triple_data.json` | Danger-triple enumeration: V_{114}, V_{134}, V_{662} factorisations and danger triples. |
