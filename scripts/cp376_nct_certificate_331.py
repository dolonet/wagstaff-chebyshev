"""cp376: complete NCT fixed-d certificate for d = 331.

Closure chain: cp360 trial+ECM on the unsplit C238 (factors 6619, 43691,
4510867, 224936385721, 1732445993139049817801), cp366 Pell half-split
Q_662 = 2 (2 P_331 - 1)(2 P_331 + 1), cp368 ECM on the halves (L-half:
p22 = 5264282380581441760369, residual P78 prime — L complete; M-half C106:
t35, no factor), cp374 GNFS on the M-half C106 (cado-nfs; factors read from
scripts/output/cp374_gnfs_results.txt line "d331_M_C106 ..."). With the
GNFS split Psi_1324 is completely factored:

  Psi_1324 = 6619 * 43691 * 4510867 * 224936385721 * p22 * P78   (L-half)
             * 1732445993139049817801 * f1 * f2                  (M-half)
...
OUT = os.path.join(HERE, "output", "cp376_certificate_331.json")
GNFS_RESULTS = os.path.join(HERE, "output", "cp374_gnfs_results.txt")

D = 331
C106 = 2039750950131097013328632659117698163772018104226200879616775771558601861099642857866569292462069720970499
KNOWN_L = [6619, 43691, 4510867, 224936385721, 5264282380581441760369]
KNOWN_M = [1732445993139049817801]
...
def main():
    fs = gnfs_factors("d331_M_C106", C106)
    psi = V(2 * D) // 6                     # 331 prime: no nested division
    P, _ = pell(D)
    L, M = 2 * P - 1, 2 * P + 1
    assert V(2 * D) == 2 * L * M
    gL, gM = math.gcd(psi, L), math.gcd(psi, M)
    assert gL * gM == psi
    assert gM == math.prod(KNOWN_M) * C106, "M-half known-factor ledger wrong"
    pL = gL // math.prod(KNOWN_L)           # the P78 residual
    factors = sorted(KNOWN_L + KNOWN_M + [pL] + fs)
    assert math.prod(factors) == psi, "product identity fails"
    print(f"[cp376] Psi_1324 C{len(str(psi))} = product of {len(factors)} "
          f"factors, identity OK")

    rec = {"d": D, "psi_4d_digits": len(str(psi)),
           "source": "cp360 trial+ECM + cp366 Pell half-split "
                     "(L=2P_331-1, M=2P_331+1) + cp368 ECM (L complete: p22 + "
                     "P78; M-half t35 no factor) + cp374 GNFS on the M-half "
                     "C106 (cado-nfs)",
        ...
        if r % 8 != 1:
            ...
            if v2 == 0:
                ...
                if not sympy.isprime(p_r):
                    ...
                    if cond_v:
                        survivors.append(r)
        print(f"[cp376] r ({fr['r_digits']}d, mod8={fr['mod8']}, "
    ...
    print(f"[cp376] d=331: {rec['status']} -> {OUT}")
