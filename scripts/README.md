...
|---|---|---|---|---|
| `survey.py` | Runs the inert-factor survey end to end | §8.5 | ~4 h @128 cores | ~64 GB |
| `rerun_segments.py` | Re-runs an explicit list of survey segments (fsync-strict) | §8.5 | tens of seconds / segment | ~1 GB |
| `build_clean.py` | Dedupes + sorts + validates the raw survey CSV | §8.5 | ~2 min | <1 GB |
| `audit.py` | Streams the clean CSV and checks it against `segment_stats.jsonl` | §8.5 | ~30 s | ~16 MB |
| `verify_sample.py` | Independently recomputes every column on a CSV slice | §8.5 | ~2 min / 1000 rows | ~50 MB |

## Primality proofs and tests

| Script | What it does | Paper section | Runtime | Dependencies |
|---|---|---|---|---|
| `chebyshev_test.py` | Chebyshev probable-prime test for W_p | §8.6 | seconds | stdlib only |
...
## Split factor analysis (Section 5 / §8.2)

| Script | What it does | Paper section | Runtime | Dependencies |
|---|---|---|---|---|
| `split_factor_census.py` | Census of 185 split factors, case breakdown, R3 verification | §8.2 | ~5 min | sympy |
| `r3_split_sweep.py` | Extended R3 sweep at r <= 10^11 with sum(1/p) tracking | §8.8 | hours @128 cores | stdlib only |

## NCT and Pell exclusion (Sections 5-6)

| Script | What it does | Paper section | Runtime | Dependencies |
|---|---|---|---|---|
| `nct_verify.py` | NCT verification (d <= 81) and Pell exclusion (d <= 43) | §5.6, §6.3 | ~1 min | sympy |
| `nct_parity_obstruction.py` | Parity obstruction analysis: which d are blocked | §5.5 | seconds | stdlib only |
| `nct_extended_verify.py` | NCT search for parity-unblocked d in [83, 200] | §8.4 | ~6 s @r<10^8 | sympy |
| `primitive_divisor_survey.py` | Primitive divisor survey and qth-power residue analysis | §8.3 | ~3 s @q<=200 | sympy |

## Inert factor analysis (Sections 6-7 / §8.5)
...
# Small inert factor census (Table 1 in §8.5)
